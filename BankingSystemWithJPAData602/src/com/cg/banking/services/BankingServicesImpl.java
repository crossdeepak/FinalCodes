...
public class BankingServicesImpl implements BankingServices {
	...
	public List<Account> getcustomerAllAccountDetails(int customerId)
		...
		List<Account> list=new ArrayList<>(customer.getAccounts().values());
		return list;
	}
	...
	AccountNotFoundException {
		...
		if(account==null)
			throw new AccountNotFoundException("AccountNo : "+accountNo+" not found.");
		Customer customer=daoServices.findOne(customerId);
		Account account=customer.getAccounts().get(accountNo);
		Map<Integer,Transaction> transaction=account.getTransactions();
		List<Transaction> list=new ArrayList<>(account.getTransactions().values());
		return list;
	}
	...
	public boolean closeAccount(int customerId, long accountNo)
		...
		if(getAccountDetails(customerId, accountNo)==null)
			...
			daoServicesAccount.delete(accountNo);
			return true;}
			else
				return false;
	}
	...
	AccountNotFoundException, BankingServicesDownException, AccountBlockedException {
		...
		if(customerId==getCustomerDetails(customerId).getCustomerId()
				&&accountNo==getAccountDetails(customerId, accountNo).getAccountNo()
				&&pinNumber==getAccountDetails(customerId, accountNo).getPinNumber())
